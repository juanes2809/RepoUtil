...
export async function POST(request: NextRequest) {
  try {
    ...
    const weight = parseFloat(process.env.MIPAQUETE_DEFAULT_WEIGHT || '1');
    const width = parseFloat(process.env.MIPAQUETE_DEFAULT_WIDTH || '20');
    const height = parseFloat(process.env.MIPAQUETE_DEFAULT_HEIGHT || '15');
    const length = parseFloat(process.env.MIPAQUETE_DEFAULT_LENGTH || '30');
